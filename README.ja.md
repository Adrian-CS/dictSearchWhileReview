...
- **デッキへの一括定義追加** (`ツール → Jisho Lookup → まとめて
  定義を追加…`): ローカルの Yomichan/Yomitan 辞書を使って、デッキ
  内のすべてのノートの定義フィールドを一括で埋めます。単語フィールド・
  定義フィールド・追記/上書きモード・定義済みカードのスキップ有無を
  選択できます。進捗バーと結果サマリー (見つかった / 見つからない /
  スキップ) を表示し、途中でキャンセルも可能です。
...
`ツール → Jisho Lookup → まとめて定義を追加…` を開きます:

1. 対象の **デッキ** を選択します。
2. **単語フィールド** (すでに単語が入っているフィールド) と
   **定義フィールド** (結果を書き込むフィールド) を選択します。
3. **追記** または **上書き** を選び、定義済みカードをスキップする
   かどうかを設定します。
4. **開始** をクリックします。進捗バーとログがリアルタイムで表示され、
   いつでも **キャンセル** で中断できます。

> 一括追加はローカル辞書 (Yomichan/Yomitan の ZIP) のみを使用します。
> `dictionaries/` フォルダに ZIP が少なくとも 1つあり、設定で
> 有効になっていることを確認してください。
